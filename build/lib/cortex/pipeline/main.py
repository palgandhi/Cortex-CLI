...
def get_target_column(data, problem_type, user_input):
    ...
    if inferred_column:
        confirmation = input(f"I found a column named '{inferred_column}'. Is this the target variable? (yes/no): ").lower().strip()
...
def run_training_pipeline(handler, model_class, problem_type, user_input):
    """
    Runs the full ML pipeline: data loading, splitting, training, and evaluation.
    """
    try:
        handler.load_data()
        data = handler.data
        ...
        # --- Handle All Supervised Learning ---
            ...
            if handler.detect_type() == "text":
                target_column = get_target_column(data, problem_type, user_input)
            ...
            if is_deep_learning:
                ...
                # Split and convert to tensors for deep learning
                ...
                test_dataloader = DataLoader(TensorDataset(X_test_tensor, y_test_tensor), batch_size=32)
                ...
                if final_model_instance.param_grid:
                    tuning_choice = input(f"Would you like to tune the hyperparameters of the {final_model_instance.name}? (yes/no): ").lower().strip()
                    if tuning_choice in ('yes', 'y'):
        ...
        # --- Final Output and Saving (common to all problem types) ---
        print("\n--- Evaluation Results ---")
